...
chkAll.change(function () {
    ...
    // 미체크 상태면(false) 개별박스도 모두 false
    chkEach.prop("checked", isChk);
    // chkEach.attr('checked',isChk);
    // for문 없이도 여러개의 체크박스를 동시에 변경할 수 있다.
...
/*********************************************** 
    ...
    체크박스의 checked 속성인 경우 true/false를 리턴해주는 것은 prop() 메서드 뿐임
    ...
    ****************************************/
            ...
            ******************************************************************/
                ...
                $.ajax({
                    ...
                    // 3. 보낼데이터,(data) - 객체형식
                    data: { mid: $("#mid").val() },
                    // 4. 전송할 데이터 타입,(dataType)
                    dataType: "html",
                    // 5. 비동기 옵션,(async)
                    // -> 비동기 옵션은 본처리를 비동기적으로 처리하겠다는 것임(기본값 : true)
...
$("#btnj").click((e) => {
    ...
    if (pass) {
        ...
        /**************************************************** 
                ...
                - 비동기 통신이란? 쉽게 말해서 페이지가 새로고쳐지지 않고 그대로 있으면서 일부분만 서버통신을 하는 것을 말한다.
                - 제이쿼리는 POST 방식으로 ajax를 처리하는 메서드를 제공한다
                [ POST 방식 Ajax 메서드 ]
                $.post(URL,data,callback)
                $.post(전송할 페이지 , 전송할 데이터, 전송 후 콜백 함수)
            ****************************************************/
        ...
        // alert('회원가입을 축하드립니다.')
        // 원래는 POST 방식으로 DB에 회원가입정보를
        // 전송하여 입력 후 DB처리완료시 성공메시지나
        // 로그인 페이지로 넘겨준다.
